...
const useGetPrice = () => {
  ...
  const getPrice = ({
    ...
    if (!pair) {
      return { amount: 0, price: 0, serviceFee: 0, marketFee: 0 }
    }
    ...
    const serviceFeeCoin = value * (serviceFee / 100)
    const serviceFeeAmount = serviceFeeCoin * price
    const marketFeeCoin = value * (marketFeePercent / 100)
    const marketFeeAmount = marketFeeCoin * price

    return {
      amount: amount,
      amountWithFees: amount - serviceFeeAmount - marketFeeAmount,
      price: price,
      serviceFee: serviceFeeAmount,
      serviceFeeCoin: serviceFeeCoin,
      marketFee: marketFeeAmount,
      marketFeeCoin: marketFeeCoin,
    }
  }
